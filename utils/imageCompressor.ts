...
/** 同 compressToBase64，但输入是 dataURL（裁剪后得到的是 dataURL，避免再转回 File）。返回 **不含 data: 前缀** 的 base64。 */
export function compressDataURLToBase64(dataUrl: string, maxSize = 800, quality = 0.85): Promise<string> {
  return new Promise((resolve, reject) => {
    const img = new Image()
    img.onload = () => {
      const scale = Math.min(1, maxSize / Math.max(img.width, img.height))
